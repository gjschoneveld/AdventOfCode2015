...
namespace Day6
{
    abstract class Command
    {
        ...
        public void Apply1(bool[,] grid)
        {
            for (int x = start.Item1; x <= end.Item1; x++)
            {
                for (int y = start.Item2; y <= end.Item2; y++)
                {
                    grid[x, y] = Apply1(grid[x, y]);
                }
            }
        }

        protected abstract bool Apply1(bool currentValue);


        public void Apply2(int[,] grid)
        {
            for (int x = start.Item1; x <= end.Item1; x++)
            {
                for (int y = start.Item2; y <= end.Item2; y++)
                {
                    grid[x, y] = Apply2(grid[x, y]);
                }
            }
        }

        protected abstract int Apply2(int currentValue);
    }

    class OnCmd : Command
    {
        protected override bool Apply1(bool currentValue)
        {
            return true;
        }

        protected override int Apply2(int currentValue)
        {
            return currentValue + 1;
        }
    }

    class OffCmd : Command
    {
        protected override bool Apply1(bool currentValue)
        {
            return false;
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var commands = commandStrings.Select(cs => Command.Parse(cs));

            int size = 1000;
            bool[,] grid1 = new bool[size, size];
