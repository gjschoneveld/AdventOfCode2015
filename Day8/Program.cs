...
namespace Day8
{
    class Program
    {
        static string Parse(string literal)
        {
            // strip off surrounding "s
            string withoutDoubleQuotes = literal.Substring(1, literal.Length - 2);

            // parse escape sequences
            StringBuilder result = new StringBuilder();
            int index = 0;

            while (index < withoutDoubleQuotes.Length)
            {
                char currentChar = withoutDoubleQuotes[index];
                ...
                // escape sequence
                char nextChar = withoutDoubleQuotes[index + 1];
                ...
                // hexadecimal
                string hexString = withoutDoubleQuotes.Substring(index + 2, 2);
                uint asciiCode = Convert.ToUInt32(hexString, 16);
                char hexChar = Convert.ToChar(asciiCode);
