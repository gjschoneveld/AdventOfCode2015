...
namespace Day12
{
    class Program
    {
        ...
        static List<Tuple<int, int>> FindRedObjectLocations(string x)
        {
            List<Tuple<int, int>> result = new List<Tuple<int, int>>();

            string searchString = "\"red\"";

            int index = x.IndexOf(searchString);
            while (index >= 0)
            {
                // search start to the left
                int start = index;
                int depth = 0;
                while (depth >= 0)
                {
        ...
        static string RemoveRedObjects(string x)
        {
            var locations = FindRedObjectLocations(x);
            var filtered = RemoveOverlappingLocations(locations);
            var sorted = filtered.Distinct().OrderBy(i => i.Item1).ToList();
            ...
            string begin = x.Substring(0, sorted.First().Item1);
            result.Append(begin);
            ...
            for (int i = 0; i < sorted.Count - 1; i++)
            {
                ...
                int inBetweenStart = sorted[i].Item2 + 1;
                int inBetweenLength = sorted[i + 1].Item1 - inBetweenStart;

                string inBetween = x.Substring(inBetweenStart, inBetweenLength);
                result.Append(inBetween);
            }

            result.Append("\"\"");

            string end = x.Substring(sorted.Last().Item2 + 1);
            result.Append(end);
