...
namespace Day3
{
    class Program
    {
        static Tuple<int, int> NextPosition(Tuple<int, int> current, char direction)
        {
            int x = current.Item1;
            int y = current.Item2;
        ...
        static void Main(string[] args)
        {
            ...
            {
                var newPos = NextPosition(current, input[i]);
            ...
            Console.WriteLine("Answer 1: {0}", visited.Count);


            HashSet<Tuple<int, int>> visited2 = new HashSet<Tuple<int, int>>();

            var currentSanta = new Tuple<int, int>(0, 0);
            var currentRobo = new Tuple<int, int>(0, 0);

            visited2.Add(currentSanta);

            for (int i = 0; i < input.Length; i++)
            {
                bool isSanta = i % 2 == 0;
                var current2 = isSanta ? currentSanta : currentRobo;

                var newPos = NextPosition(current2, input[i]);

                if (!visited2.Contains(newPos))
                {
                    visited2.Add(newPos);
                }
            ...
            Console.WriteLine("Answer 2: {0}", visited2.Count);
